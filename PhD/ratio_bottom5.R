...
ratio_bottom5 = function(t.total,nt,vt,nx,ny,index,var,nc,ns,alpha=0.05){
...
#  dim(syn.var) = c(nx*ny,t.total,ns)
  ...
  quantile = apply(var, 1,  quantile, probs= c(alpha), na.rm=T )  ##dim=x*y
  ...
  if (ns!=1){
    ...
    for (k in 1:ns){
        ...
        syn.quantile = apply(syn.var, 1,  quantile, probs= c(alpha), na.rm=T )
      for (j in 1:nc){
